...
import models
import data_loader
from processEmodb import Emodb_LABEL
...
def parse_args():
    ...
    parser.add_argument('-i', '--impro_or_script',default='Emodb',type=str,help='select features')
    ...
    # model
    parser.add_argument('-m', '--model',default='CTMAM_EMODB',type=str,help='specify models')
    ...
    # training
    parser.add_argument('--seed',default=987654,type=int,help='random seed')#987654,4444
    parser.add_argument('-b','--batch_size',default=128,type=int,help='batch size')#128
    parser.add_argument('-l','--learning_rate',default=0.001,type=float,help='learning rate')#0.001
    ...
    parser.add_argument('-d', '--datadir',default='data/EMO-DB',type=str,help='models')
...
def get_loss(model, criterion, x,y,alpha):
    if alpha > 0:
        ...
        #修改前
        loss = mixup_criterion(criterion, out, ya, yb, lam)#lam * criterion(pred, y_a) + (1 - lam) * criterion(pred, y_b)
        return loss
    else:
        if torch.cuda.is_available():
            x = x.cuda()
...
def train(kws):
    ...
    print(shape)
    model = getattr(models, kws['model'])(shape=shape)  # 调用模型 getatter就是返回对象的属性值，这里相当于直接去拿模型的类
    print(summary(model,torch.zeros((1, 1, *shape))))#打印模型
    ...
    MODEL_PATH = f'{kws["datadir"]}/model_Emodb_{kws["model"]}_{filename}.pth'
...
if __name__ == '__main__':
    ...
    args = parse_args()#设置各种参数，值就是defult
    kws = config(args)#变成键值对
    ...
    filename = f'{kws["features_to_use"]}_Emodb'
    if kws['featuresFileName'] is None:
        kws['featuresFileName'] = f'{kws["datadir"]}/features_{filename}.pkl'    #featuresFileName：'data/features_mfcc_impro.pkl'


    if not os.path.exists(kws["datadir"]): os.system(f'mkdir -p {kws["datadir"]}')#没有这个目录就创建目录

    with open(kws["datadir"] + '/last_emodb.json', 'w') as f:
        del kws['config']
    ...
    if kws['loadFeatures'] and os.path.exists(kws['featuresFileName']):
        ...
        from processEmodb import process_Emodb

        datasets_Emodb=r'/mnt/data/LYX/dataset/Emodb/download/wav'
        train_X, train_y, val_dict, info = process_Emodb(datasets_Emodb, Emodb_LABEL, **kws)
    ...
    target_names = ['Neutral','sad','Angry','Happy','Boredom','Disgust','Fear']
